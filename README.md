...
- **📱 严苛的身份认证机制**：支持注册后的强制 **邮箱验证**，并将重要高危操作收敛于认证之后，全面支持 **2FA (基于 Google Authenticator 的二次验证码)** 登录与现代化的 **无密码 Passkey 通行密钥**，将内部账户被盗风险降至最低。
- **🛡️ 账号防爆破与单点登录 (Session Control)**：支持同账号多地登录**自动互踢** (Concurrent Login Prevention)。内置密码试错防爆破系统，连续输入错误 3 次即锁定账号 15 分钟。
- **🔄 找回不暴露机制 (双链路安全找回)**：支持通过邮箱找回“激活码”和“管理员密码”。当找回激活码时，系统会直接吊销旧码并发放新码，彻底堵死已泄露激活码的二次滥用。
- **📊 智能数据大盘与用户管理 (Analytics & User Management)**：内置图形化管理员看板 (`/admin`)，实时双轨呈现 **今日新增激活量** 与 **异常拦截攻击量**。支持管理员针对普通用户重置密码、权限升降级，支持一秒生成千张授权码，并一键导出为 CSV 报表，完美对接发卡网/经销商。
- **🚀 极致轻量化与全栈性能优化**：前端采用纯静态构建 (Next.js Static Export) 和 **SWR 自动缓存去重架构**；后端采用极小体积的 .NET Alpine 镜像，底层集成 **IMemoryCache 内存级高速缓存** 及 Entity Framework 的 `.AsNoTracking()` 无追踪查询优化，高并发下依然稳如泰山。
- **🐳 容器化黑盒防扫描**：通过 Docker Compose 部署，数据库与后端 API 的端口被完全封锁在虚拟内网中。唯一对外的窗口由 Nginx (80/443) 严格把守，并 **强制全链路 HTTPS**。
